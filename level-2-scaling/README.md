...
> **Теория уровня — в `CURRICULUM.md` → «Уровень 2»**: scale up vs scale out, алгоритмы балансировки, анатомия upstream в nginx.conf, вопросы с собеседований. Здесь она не дублируется. Легенда `[голова]`/`[руки]` — в START_HERE.md.
...
- В `nginx.conf` — блок `upstream backend` с тремя серверами (round-robin)
- В `docker-compose.yml` — три сервиса `backend_1`, `backend_2`, `backend_3` вместо одного `backend`
...
| `k6 run load-tests/balancer-test.js` | Тест балансировки |
