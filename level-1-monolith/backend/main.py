"""
Bulletin Board — REST API v2
Что нового по сравнению с "просто CRUD":
  - Регистрация и логин через JWT
  - Объявления привязаны к пользователям (user_id)
  - Схема БД управляется Alembic — не CREATE TABLE IF NOT EXISTS
  - Connection pool: не новый коннект на каждый запрос
"""
import os
import time
    ...
    for attempt in range(10):
        try:
            engine = create_engine(
                ...
                pool_size=5,
                max_overflow=10,
                pool_pre_ping=True,  # проверяет коннект перед выдачей из пула
            )
