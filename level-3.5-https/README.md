...
> **Это [руки]** — практический маршрут уровня: команды, эксперименты, проверки. Нужна сессия с VM (для Let's Encrypt — публичный домен).
> **Теория уровня — в `CURRICULUM.md` → «Уровень 3.5»**: зачем HTTPS, Traefik vs Certbot, ACME, вопросы с собеседований. Здесь она не дублируется. Легенда `[голова]`/`[руки]` — в START_HERE.md.
