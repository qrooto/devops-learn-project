"""
Bulletin Board API v2 — Monitoring
Что добавлено к уровню 3 (кэш):
  - prometheus-fastapi-instrumentator: автоматически добавляет /metrics
  - Prometheus собирает: http_requests_total, http_request_duration_seconds, in_progress_requests
  - Grafana строит дашборд на основе этих метрик

Зачем /metrics?
  Без метрик ты слепой — не знаешь сколько запросов, какое время ответа, где ошибки.
  С Prometheus + Grafana видишь деградацию в реальном времени.
"""
import os
import time
import json
...
# Prometheus инструментация: добавляет /metrics с HTTP-метриками.
# Запускается до add_middleware — иначе метрики не будут собираться корректно.
