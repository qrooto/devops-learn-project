...
> **Это [руки]** — практический маршрут уровня: команды, эксперименты. Нужна сессия с VM (minikube + ArgoCD).
> **Теория уровня — в `CURRICULUM.md` → «Уровень 8»**: что такое GitOps, анатомия ArgoCD Application, GitOps vs push-CI/CD, вопросы с собеседований. Здесь она не дублируется. Легенда `[голова]`/`[руки]` — в START_HERE.md.
