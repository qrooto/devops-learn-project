...
> **Это [руки]** — практический маршрут уровня: команды, эксперименты, поломки. Нужна сессия с VM (или две VM для multi-server).
> **Теория уровня — в `CURRICULUM.md` → «Уровень 10»**: зачем configuration management, анатомия inventory/playbook/роли/Vault, вопросы с собеседований. Здесь она не дублируется. Легенда `[голова]`/`[руки]` — в START_HERE.md.
