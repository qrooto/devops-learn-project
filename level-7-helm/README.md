# Уровень 7 — Helm: пакетный менеджер Kubernetes

> **Это [руки]** — практический маршрут уровня: команды, эксперименты, поломки. Нужна сессия с VM (minikube).
> **Теория уровня — в `CURRICULUM.md` → «Уровень 7»**: зачем Helm, анатомия Chart.yaml/values.yaml/шаблонов, вопросы с собеседований. Здесь она не дублируется. Легенда `[голова]`/`[руки]` — в START_HERE.md.
