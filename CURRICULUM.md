...
**Требования:** Ubuntu 22.04 / Debian 12, 4+ CPU, 8GB RAM, 40GB SSD.
...
EXPOSE 8000
```
**EXPOSE — документация, не конфигурация.** Эта инструкция не открывает порт и не делает ничего функционального. Она говорит читателю Dockerfile: "этот контейнер слушает на 8000". Реальное открытие порта происходит при `docker run -p 8000:8000` или в `docker-compose.yml` через `ports:`.
...
**Exec form** (массив строк) vs **Shell form** (строка):
- Exec form `["uvicorn", ...]` — процесс запускается напрямую как PID 1. Получает сигналы (SIGTERM при `docker stop`) и может корректно завершиться.
- Shell form `uvicorn main:app ...` — запускается через `/bin/sh -c "..."`. Uvicorn будет дочерним процессом sh. `docker stop` отправит SIGTERM sh, а не uvicorn — приложение может не успеть корректно завершиться. Всегда используй exec form.
...
**CMD vs ENTRYPOINT:**
- `ENTRYPOINT` — неизменяемая точка входа (например `python3`). Аргументы передаются через CMD или при `docker run`.
- `CMD` — команда по умолчанию, можно переопределить при `docker run`: `docker run bulletin-backend python3 manage.py migrate` — заменит CMD.
- Часто используют `ENTRYPOINT ["python3"]` + `CMD ["main.py"]`, или только `CMD` с полной командой как у нас.
...
RUN user  ← 6. Безопасность
USER      ← 7. Переключить на непривилегированного пользователя
EXPOSE    ← 8. Документация
CMD       ← 9. Команда запуска
```
...
Что искать в логах:
```
bulletin-board-postgres  | database system is ready to accept connections
bulletin-board-backend   | Running database migrations...
bulletin-board-backend   | INFO  [alembic.runtime.migration] Running upgrade -> 001
bulletin-board-backend   | Application startup complete.
```
...
**Задание:** Измени `SECRET_KEY` в docker-compose.yml, перезапусти бэкенд, попробуй использовать старый токен. Что произошло и почему?
...
### Анатомия nginx.conf (балансировщик)
...
- По умолчанию (ничего не указано) — **round-robin**: запросы по кругу. backend_1 → backend_2 → backend_3 → backend_1...
...
`keepalive 32` — пул до 32 постоянных HTTP-соединений к каждому бэкенду. Без keepalive: для каждого запроса — новое TCP-соединение (3-way handshake: SYN → SYN-ACK → ACK). При 100 RPS это 100 рукопожатий в секунду. С keepalive — соединение переиспользуется, лишние ~1ms на каждый запрос экономятся.

```nginx
    server backend_1:8000 weight=3;
    server backend_2:8000 weight=1;
```
`weight` — вес при round-robin. backend_1 получит 75% запросов, backend_2 — 25%. Нужно когда серверы разной мощности. В нашем случае все одинаковые — weight не используем.

```nginx
        proxy_next_upstream error timeout http_502 http_503;
        proxy_next_upstream_tries 3;
```
`proxy_next_upstream` — если бэкенд вернул ошибку, повторить запрос на следующий сервер:
- `error` — ошибка TCP-соединения (бэкенд недоступен)
- `timeout` — превышено время ожидания
- `http_502`, `http_503` — бэкенд вернул 502/503

`proxy_next_upstream_tries 3` — максимум 3 попытки. Без ограничения — при падении всех бэкендов Nginx будет крутиться бесконечно.
...
В `docker-compose.yml` измени `SECRET_KEY` у `backend_2` на другое значение. Запусти. Залогинься (токен выдан на backend_1 или backend_3). Потом делай запросы — каждый третий будет падать с 401, когда попадёт на backend_2.
...
```
`run:` — выполнить shell-команды. `|` — многострочная строка. `env:` — переменные окружения только для этого шага. Тесты запускаем с SQLite чтобы не поднимать PostgreSQL — для unit-тестов достаточно.
...
```
`needs: test` — этот job запустится только после успешного завершения job `test`. `if: github.ref == 'refs/heads/main'` — запускать сборку только при пуше в `main`, не при каждом PR. Экономит минуты GitHub Actions и не засоряет registry тестовыми образами.
...
```
`${{ secrets.GITHUB_TOKEN }}` — автоматически создаваемый токен с доступом к ресурсам репозитория. Не нужно создавать вручную. Не путай с `secrets.GHCR_TOKEN` — тот нужен для доступа к другим репозиториям. `${{ github.actor }}` — имя пользователя сделавшего пуш.
...
```yaml
      - name: Build and push Docker image
        ...
        with:
          ...
          push: true
          tags: |
            ghcr.io/${{ github.repository_owner }}/bulletin-backend:latest
            ghcr.io/${{ github.repository_owner }}/bulletin-backend:${{ github.sha }}
```
Два тега: `latest` — всегда последняя версия, `${{ github.sha }}` — конкретный коммит (40-символьный хэш). Тег по sha позволяет откатиться к точной версии: `docker pull ghcr.io/user/bulletin-backend:abc1234`.
    ...
    steps:
      - name: Deploy via SSH
        ...
        with:
          ...
          script: |
            cd /home/ubuntu/bulletin-board
            docker compose pull
            docker compose up -d --no-deps backend_1
            sleep 10
            docker compose up -d --no-deps backend_2
            sleep 10
            docker compose up -d --no-deps backend_3
```
`secrets.SSH_PRIVATE_KEY` — приватный ключ для SSH. Хранится зашифрованным в GitHub Secrets. Action устанавливает его во временный файл, подключается к серверу, выполняет `script`, удаляет ключ.

Rolling update: обновляем backend_1, ждём 10 секунд (nginx убедится что он healthy), обновляем backend_2, ещё 10 секунд, backend_3. Во время обновления два бэкенда всегда работают. Пользователи не замечают деплой.
...
```
`variables:` — переменные доступные во всех jobs. `$CI_REGISTRY_IMAGE` — автоматическая переменная GitLab: адрес образа в GitLab Registry (`registry.gitlab.com/user/project`). `$CI_COMMIT_SHORT_SHA` — короткий хэш коммита (8 символов).
...
test:
  ...
  only:
    ...
    - main
```
`image:` — Docker-образ в котором запускается job (GitLab Runner запускает все jobs в контейнерах). `only:` — запускать только для MR и ветки main. Аналог `if:` в GitHub Actions.
...
build:
  ...
  only:
    - main
```
`image: docker:24-dind` — Docker-in-Docker: запускаем Docker внутри Docker-контейнера Runner-а. Нужно для сборки образов в GitLab CI. `services: docker:24-dind` — сайдкар-контейнер с Docker daemon. `$CI_REGISTRY_USER`, `$CI_REGISTRY_PASSWORD`, `$CI_REGISTRY` — автоматические переменные GitLab для авторизации в встроенном Registry.
...
deploy:
  ...
  when: manual
```
`environment:` — привязка к GitLab Environments. В UI видно какая версия сейчас на production, история деплоев. `when: manual` — деплой не автоматический, требует нажатия кнопки в GitLab UI. Страховка от случайных деплоев.
...
3. `deploy` — SSH на сервер, делает `docker compose pull && docker compose up -d`
...
- `GHCR_TOKEN` — Personal Access Token с правом `write:packages`
...
Открой `http://localhost` — GitLab UI.
...
docker compose exec runner gitlab-runner register \
  --non-interactive \
  --url http://gitlab \
  --registration-token <TOKEN_FROM_GITLAB_UI> \
  --executor docker \
...
Добавь синтаксическую ошибку в `backend/Dockerfile`. Запушь. Pipeline должен упасть на стадии `build`. Верни и запушь снова.
...
| Команда | Описание |
|---------|---------|
| `docker pull ghcr.io/<user>/bulletin-backend:<tag>` | Скачать образ из registry |
...
```yaml
  template:
    ...
    spec:
      containers:
        - name: backend
          image: ghcr.io/user/bulletin-backend:v1.0.0
```
...
```yaml
          env:
            - name: DATABASE_URL
              valueFrom:
                secretKeyRef:
                  name: bulletin-board-secrets
                  key: database-url
            - name: REDIS_URL
              valueFrom:
                configMapKeyRef:
                  name: bulletin-board-config
                  key: redis-url
```
...
`port: 80` — порт на котором Service доступен внутри кластера. `targetPort: 8000` — порт на Pod-е куда перенаправить. Другие Pod-ы обращаются к `http://backend.bulletin-board.svc.cluster.local:80`.

**Типы Service:**
- `ClusterIP` (по умолчанию) — доступен только внутри кластера. Подходит для internal сервисов (backend, redis, postgres).
- `NodePort` — открывает порт на каждой Node кластера (30000-32767). Доступен снаружи по `<node-ip>:<nodeport>`. Для разработки.
- `LoadBalancer` — создаёт облачный балансировщик (AWS ELB, GCP LB). Дорого, для production.
...
data:
  ...
  log-level: "INFO"
  allowed-origins: "https://bulletin-board.example.com"
```
ConfigMap — несекретная конфигурация. `data:` — key-value пары строк. Можно использовать как переменные окружения (`configMapKeyRef`) или монтировать как файлы (`configMap` volume). Если изменить ConfigMap — Pod-ы не перезапустятся автоматически (нужно сделать rolling restart).
...
type: Opaque
data:
  database-url: cG9zdGdyZXNxbDovL3...  # base64
  jwt-secret: c2VjcmV0...               # base64
```
...
# postgresql://postgres:password@postgres:5432/bulletin_board
```
...
```bash
kubectl create secret generic bulletin-board-secrets \
  --from-literal=jwt-secret="$(openssl rand -hex 32)" \
  --from-literal=database-url="postgresql://postgres:${DB_PASSWORD}@postgres:5432/bulletin_board" \
  -n bulletin-board
```
...
ls k8s/
# namespace.yaml  postgres.yaml  backend.yaml  nginx.yaml  ingress.yaml  hpa.yaml
```

Прочитай `k8s/backend.yaml`:
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
```bash
# Собрать образ в контексте minikube
eval $(minikube docker-env)
docker build -t bulletin-backend:latest ./backend
...
kubectl set image deployment/backend backend=bulletin-backend:v2 -n bulletin-board
...
**Трейсы** — путь запроса: nginx(2ms) → backend(40ms) → postgres(35ms).
...
`evaluation_interval` — как часто вычислять alerting rules. Должен быть ≤ scrape_interval.

`external_labels` — добавляются ко всем метрикам при отправке в remote storage (Thanos, Cortex). Позволяют различать метрики из разных кластеров.
    ...
    static_configs:
      - targets:
        ...
        labels:
          ...
          tier: application
```
`job_name` — имя задачи сбора. Добавляется к каждой метрике как label `job="backend"`. Переопределяет `scrape_interval` из global для этого job. `metrics_path` — по умолчанию `/metrics`. У нашего бэкенда путь `/api/metrics`. `static_configs` — статический список серверов. Альтернатива: `file_sd_configs` (читать из файла), `dns_sd_configs` (DNS SRV записи), `kubernetes_sd_configs` (K8s API).
  ...
  - job_name: 'cadvisor'
    ...
    metric_relabel_configs:
      ...
      - source_labels: [__name__]
        ...
        action: keep
```
`metric_relabel_configs` — трансформация метрик ПОСЛЕ сбора. `source_labels` → `target_label`: переименование label. `action: keep` с `regex` — оставлять только метрики подходящие под паттерн. cAdvisor генерирует сотни метрик — большинство не нужны, `keep` фильтрует только нужные. Это экономит память Prometheus.
...
```
Куда отправлять сработавшие alerts. Alertmanager — отдельный сервис который группирует, дедуплицирует и маршрутизирует alerts в Telegram/Slack/PagerDuty.
...
```bash
# Вызвать эндпоинт который намеренно накапливает память
for i in $(seq 1 100); do
  curl -s http://localhost/api/debug/leak > /dev/null
done
```
...
| Команда | Описание |
|---------|---------|
| `curl http://localhost/api/metrics` | Метрики бэкенда |
...
1. Получает webhook от Grafana при алерте
...
- Type: Webhook
- URL: `http://agent:8001/webhook/alert`
...
Ключевые части:
- `collect_metrics()` — PromQL запросы через Prometheus API
- `collect_logs()` — LogQL запросы через Loki API
- `diagnose()` — формирует промпт и вызывает Claude API
- `handle_action()` — выполняет одобренные действия (restart, scale)
...
apiVersion: v2
name: bulletin-board
description: A Bulletin Board application Helm chart
type: application
version: 0.3.1
appVersion: "1.2.0"
```
`apiVersion: v2` — версия формата Chart (v2 для Helm 3, v1 для Helm 2 — уже не используется). `name` — имя chart. Используется в метаданных релиза. `version` — версия самого chart (инфраструктурные изменения). `appVersion` — версия приложения (код). Они независимы: можно изменить конфиг nginx в chart (version 0.3.2) не меняя код приложения (appVersion 1.2.0). `type: application` vs `library` — library chart не деплоится сам, только переиспользуется другими.
...
image:
  repository: ghcr.io/user/bulletin-backend
  tag: "latest"
...
postgresql:
  ...
  auth:
    ...
    password: ""   # обязательно переопределить!
...
- `--set image.tag=v1.2.3` — одно значение из командной строки
- `--set-string` — принудительно как строка (для тегов которые выглядят как числа)

Значения вложены иерархически. В шаблоне доступны как `.Values.image.repository`. Всегда документируй обязательные параметры (без default) в comments.
...
kind: Deployment
metadata:
  name: {{ include "bulletin-board.fullname" . }}
  labels:
    {{- include "bulletin-board.labels" . | nindent 4 }}
```
`{{ }}` — блоки шаблонизатора Go (Go template). `include "bulletin-board.fullname" .` — вызов именованного шаблона из `templates/_helpers.tpl`. Точка `.` — текущий контекст (включает `.Values`, `.Release`, `.Chart`).

`{{- include ... | nindent 4 }}` — дефис убирает пробелы/переносы перед блоком. `nindent 4` — добавляет отступ 4 пробела к каждой строке вывода.
    ...
    spec:
      containers:
        - name: backend
          ...
          {{- end }}
```
`.Values.replicaCount` — значение из values.yaml. `{{- if .Values.resources }}...{{- end }}` — условный блок: добавлять `resources:` только если определено в values. `toYaml` — преобразует Go-объект обратно в YAML-строку.
...
```yaml
          env:
            - name: DATABASE_URL
              valueFrom:
                secretKeyRef:
                  name: {{ include "bulletin-board.fullname" . }}-secrets
                  key: database-url
            ...
            {{- end }}
```
`{{- range .Values.extraEnv }}` — итерация по массиву. Позволяет добавлять произвольные env-переменные через values.yaml без изменения шаблона. `quote` — оборачивает значение в кавычки (важно для значений которые могут быть числами).
...
{{- end }}
```
`_helpers.tpl` — файл начинающийся с `_` не рендерится как манифест. Только определяет переиспользуемые шаблоны. `define "имя"` — объявление. `include "имя" .` — вызов. `trunc 63` — обрезать до 63 символов (лимит K8s для имён). `trimSuffix "-"` — убрать хвостовой дефис если имя обрезалось.
...
| Команда | Что делает | Пример |
|---------|-----------|--------|
| `helm install` | Установить chart | `helm install bulletin ./chart -f values.prod.yaml` |
| `helm upgrade` | Обновить релиз | `helm upgrade bulletin ./chart --set image.tag=v1.2.3` |
| `helm rollback` | Откатить | `helm rollback bulletin 1` |
| `helm list` | Список релизов | `helm list -n bulletin-board` |
| `helm history` | История релизов | `helm history bulletin` |
| `helm template` | Показать сгенерированные манифесты | `helm template bulletin ./chart` |
...
```bash
cat chart/templates/backend-deployment.yaml
```
...
```bash
helm template bulletin ./chart
helm template bulletin ./chart -f values.staging.yaml
```

**Шаг 3 — Установить**

```bash
helm install bulletin ./chart \
  --namespace bulletin-board \
  --create-namespace \
  -f values.yaml
...
```bash
helm upgrade bulletin ./chart \
  --set image.tag=v1.2.3 \
  -n bulletin-board

helm history bulletin -n bulletin-board
```

**Шаг 5 — Blue-Green деплой**

```bash
# Деплой "синей" версии (текущая)
helm install blue ./chart --set color=blue --set image.tag=v1.0.0

# Деплой "зелёной" версии (новая)
helm install green ./chart --set color=green --set image.tag=v1.1.0

# Переключить трафик на зелёную (изменить Service selector)
kubectl patch service bulletin-board -p '{"spec":{"selector":{"color":"green"}}}'

# Если всё хорошо — удалить синюю
helm uninstall blue

# Если что-то пошло не так — переключить обратно
kubectl patch service bulletin-board -p '{"spec":{"selector":{"color":"blue"}}}'
...
`source.path: level-8-gitops/k8s` — поддиректория в репозитории с манифестами. Один репо может содержать несколько Application-ов для разных сервисов/окружений.
...
```bash
cd level-8-gitops
# Посмотреть Application manifest
...
Измени `replicas: 3` на `replicas: 5` в `k8s/backend.yaml`, закоммить и запушь в Git:
...
#### versions.tf / main.tf (блок terraform)
...
`required_providers` — провайдеры которые нужно скачать. `terraform init` скачивает их в `.terraform/`. `source` — реестр провайдера (`registry.terraform.io/digitalocean/digitalocean`). `version = "~> 2.0"` — **pessimistic constraint operator**: принимает 2.x (2.1, 2.34) но не 3.0. Никогда не убирай версионирование провайдеров — их breaking changes сломают твой код.

`backend "s3"` — где хранить state файл. Локальный state (по умолчанию `terraform.tfstate` в текущей директории) — нельзя использовать в команде. Нет locking — двое запустят `apply` одновременно, state повредится. `dynamodb_table` — DynamoDB-таблица для distributed locking (только один `apply` в единицу времени). `encrypt: true` — шифровать state в S3 (он содержит пароли!).
...
variable "ssh_key_id" {
  type        = number
  description = "DigitalOcean SSH key ID (get from 'doctl compute ssh-key list')"
  # no default — обязательная переменная, terraform потребует ввести при apply
}

variable "db_password" {
  type      = string
  sensitive = true   # не показывать в выводе terraform plan/apply
}
```
`type` — string, number, bool, list(string), map(string), object({...}). Строгая типизация предотвращает ошибки. `sensitive = true` — значение скрывается в выводах. Всегда помечай пароли и токены.

`validation` блок — проверка значения. Ошибка до любых API-вызовов. Значительно лучше чем получить непонятную ошибку от провайдера.
...
resource "digitalocean_droplet" "web" {
  name     = "bulletin-board-${var.environment}"
  size     = var.droplet_size
  image    = "ubuntu-22-04-x64"
  region   = "fra1"
  ssh_keys = [var.ssh_key_id]
  tags     = ["bulletin-board", var.environment]
...
```
`resource "TYPE" "LOCAL_NAME"` — TYPE — тип ресурса провайдера, LOCAL_NAME — имя внутри Terraform (не имя в облаке). Обращение: `digitalocean_droplet.web.ipv4_address`.

`templatefile("path", {vars})` — рендерит файл как шаблон с переменными. Полезно для cloud-init скриптов.
...
resource "digitalocean_firewall" "web" {
  ...
  inbound_rule {
    protocol         = "tcp"
    port_range       = "22"
    source_addresses = ["10.0.0.0/8"]  # только из внутренней сети
  }
...
```
`digitalocean_droplet.web.id` — **implicit dependency**: Terraform видит что firewall зависит от droplet (использует его id) и создаёт droplet ПЕРВЫМ. Не надо объявлять порядок явно. Явная зависимость: `depends_on = [digitalocean_droplet.web]` — только если implicit не работает.
...
output "server_ssh" {
  value       = "ssh ubuntu@${digitalocean_droplet.web.ipv4_address}"
  description = "SSH command"
}

output "db_password" {
  value     = var.db_password
  sensitive = true  # не показывать в terraform output без --raw
}
```
Outputs — значения которые Terraform выводит после `apply`. Используются: для отображения пользователю ("вот IP сервера"), как входные данные для другого Terraform модуля, в CI/CD для передачи IP следующему шагу.
      ...
      "instances": [
        {
          "attributes": {
            "id": 12345678,
            "ipv4_address": "123.456.789.0",
            "name": "bulletin-board-staging"
          }
        }
      ]
    }
...
```bash
terraform init
# Скачает провайдеры в .terraform/
```
...
State — память Terraform о том что уже создано. Если state потерян — Terraform не знает о существующих ресурсах → опасность дублирования.

```bash
# В production state хранят в S3 или Terraform Cloud, не локально
# backend "s3" {
#   bucket = "my-terraform-state"
...
Измени `size = "s-4vcpu-8gb"`. Запусти `terraform plan` — увидишь что Droplet будет пересоздан (не просто изменён). `terraform apply`.
...
# Это означает дублирование ресурсов и деньги

mv terraform.tfstate.backup terraform.tfstate
```

**Поломка 2 — Изменить ресурс в консоли вручную**

Зайди в DigitalOcean/AWS Console, измени size droplet вручную. Запусти `terraform plan`. Увидишь:
```
~ digitalocean_droplet.web
    size: "s-4vcpu-8gb" → "s-2vcpu-4gb"
```

Terraform хочет вернуть к состоянию из `.tf` файлов. Если применить — droplet будет пересоздан.
...
```
`all` — корневая группа, включает все хосты. `children` — вложенные группы. `hosts` — конкретные серверы. `vars` — переменные уровня группы (применяются ко всем хостам в группе).

`ansible_host` — реальный IP или hostname. Имя хоста (`web1`) — псевдоним для читаемости. `ansible_port` — SSH порт (по умолчанию 22). `ansible_user` — SSH пользователь. `ansible_ssh_private_key_file` — путь к приватному ключу.
...
```yaml
- name: Setup and deploy Bulletin Board
  hosts: web
  become: yes
  become_method: sudo
  gather_facts: yes
  serial: 1
```
`hosts: web` — применять к группе `web` из inventory. `all` — ко всем. `web1` — к конкретному хосту. `web:!web1` — ко всем в web кроме web1.

`become: yes` — выполнять задачи от имени другого пользователя (по умолчанию root через sudo). `become_method: sudo` — как именно повышать привилегии (sudo, su, pbrun, pfexec).

`gather_facts: yes` — перед выполнением задач Ansible собирает факты о хосте: OS, версия, IP-адреса, память, CPU. Доступны как `ansible_os_family`, `ansible_memtotal_mb`, `ansible_default_ipv4.address`. `no` — если не нужны, экономит ~2 секунды.

`serial: 1` — выполнять плейбук по одному серверу за раз (не параллельно). Полезно для rolling deploy: обновить web1, убедиться что работает, обновить web2. По умолчанию все серверы параллельно.

```yaml
  vars:
    app_dir: /home/ubuntu/bulletin-board
    docker_compose_version: "2.24.0"

  vars_files:
    - secrets.yml
```
`vars` — переменные плейбука. `vars_files` — подключить файл с переменными (например зашифрованный через Vault).
...
```yaml
  ...
  tasks:
    - name: Install required packages
      ...
      tags: [packages]
```
`pre_tasks` — выполняются до `tasks` (даже до roles). Используется для проверок и подготовки. `changed_when: false` — не считать задачу "изменившей" систему даже если команда что-то вывела. Важно для idempotency-отчётов.

`state: present` — установить если не установлено. `state: latest` — установить и обновить до последней версии. `state: absent` — удалить. `update_cache: yes` — `apt-get update` перед установкой.

`tags: [packages]` — помечаем задачу. Можно запускать только помеченные: `ansible-playbook deploy.yml --tags packages`. Или пропускать: `--skip-tags packages`.
...
```yaml
    - name: Create app directory
      file:
        ...
        mode: '0755'
```
`file` модуль — управление файлами и директориями. `state: directory` — создать директорию. `state: file` — создать/изменить файл. `state: absent` — удалить. `state: touch` — создать пустой файл или обновить timestamp.

`mode: '0755'` — права доступа в восьмеричном формате. Кавычки обязательны чтобы YAML не интерпретировал как число.
...
```
`template` vs `copy`:
- `copy` — копирует файл как есть. Подходит для бинарников, сертификатов.
- `template` — рендерит Jinja2-шаблон перед копированием. В `.j2` файле можно писать `{{ app_dir }}`, `{% if environment == 'production' %}...{% endif %}`.

`notify: Restart services` — триггер для handler. Если задача изменила что-то (`changed`) — в конце плейбука будет вызван handler "Restart services". Если изменений нет — handler не вызывается. Если 5 задач уведомили один handler — он вызывается один раз в конце.
...
```
`environment` в задаче — переменные окружения только для этой команды. Не путай с `vars` (переменные Ansible) и `environment` в playbook (переменные для всех задач).
  ...
  handlers:
    - name: Restart services
      docker_compose:
        ...
        restarted: yes
```
Handlers — специальные задачи вызываемые через `notify`. Выполняются в конце плейбука (после всех tasks), один раз, в порядке объявления. Классический пример: изменил nginx.conf → notify "Reload nginx" → nginx перезагружается один раз в конце, не после каждого изменения файла.
...
# Посмотреть без редактирования
ansible-vault view secrets.yml

# Зашифровать одно значение (вставить в обычный YAML)
...
**В CI/CD** vault-пароль хранится в секретах (GitHub Secrets, GitLab CI Variables), записывается во временный файл перед запуском, удаляется после.
...
| `ansible-vault encrypt` | Зашифровать файл с секретами | `ansible-vault encrypt secrets.yml` |
...
**Шаг 3 — Изучить плейбук**

```bash
cat playbooks/setup.yml
```

```yaml
- name: Setup bulletin board server
  hosts: all
  become: yes    # sudo
  ...
  tasks:
    ...
    - name: Copy docker-compose.yml
      copy:
        src: ../../level-1-monolith/docker-compose.yml
        dest: /home/ubuntu/bulletin-board/docker-compose.yml
        owner: ubuntu

    - name: Start services
      docker_compose:
        project_src: /home/ubuntu/bulletin-board
        state: present
```
...
```bash
ansible-playbook -i inventory.yml playbooks/setup.yml --check --diff
```

Покажет что изменится — без реальных изменений.
...
Ansible применит на все серверы параллельно.

**Шаг 6 — Ansible Vault для секретов**
...
# Файл теперь зашифрован — можно коммитить в Git
cat secrets.yml
# $ANSIBLE_VAULT;1.1;AES256
# ...

# Использовать в плейбуке
ansible-playbook -i inventory.yml deploy.yml --ask-vault-pass
```

**Шаг 7 — Роли: организация плейбуков**

```bash
ls roles/
# docker/      — устанавливает Docker
# backend/     — деплоит бэкенд
# postgres/    — настраивает PostgreSQL
# nginx/       — конфигурирует Nginx
```

Роли — модульный способ организации задач. Можно переиспользовать: роль `docker` подходит для любого проекта.
...
| 6.5 | `docker compose up -d agent` | Claude API |
| 7 | `helm install bulletin ./chart` | Helm |
| 8 | `kubectl apply -f argocd/` | ArgoCD |
| 9 | `terraform apply` | Terraform |
| 10 | `ansible-playbook playbooks/setup.yml` | Ansible |
