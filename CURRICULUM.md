...
>
> **Это [голова]** — весь этот файл читается без терминала (метро, короткие сессии): «Зачем», «Как это работает», «Анатомия», «На собеседовании спросят». Вся практика — по ссылкам `→ Практика [руки]` в README уровней: шаги с командами, «Что сломать намеренно», справочники команд, Security Block, Troubleshooting. Контент не дублируется: теория живёт здесь, руки — в README. Подробнее — START_HERE.md.
...
### На собеседовании спросят — Уровень 5.5
...
> **→ Практика [руки]:** `level-6-monitoring/README.md` — Шаги 1-10 (метрики бэкенда, PromQL, Grafana-дашборд, нагрузка на графиках, Loki/LogQL, traces, Alertmanager, алерт в Telegram), «Что сломать намеренно» (memory leak, убить Prometheus, потерять логи), справочник команд.
...
Чеклист умений, Best Practices Checklist и коммит — в `level-6-monitoring/README.md`. Пройди их перед переходом.
...
> **→ Практика [руки]:** `level-6.5-ai-agent/README.md` — Шаги 1-10 (Telegram-бот, Claude API ключ, .env, туннель WireGuard до VPS, Alertmanager webhook, запуск агента, тесты вебхуком и реальным алертом, разбор промпта, безопасность действий агента).
...
Чеклист умений и коммит — в `level-6.5-ai-agent/README.md`.
