...
> **Это [руки]** — практический уровень: настройка VPS выполняется один раз, целиком за терминалом. Легенда `[голова]`/`[руки]` — в START_HERE.md.
...
**Среда:** Ubuntu 24.04 LTS

ℹ️ **Новое в 24.04:** системный `pip install` на голый Python теперь заблокирован (PEP 668, ошибка `externally-managed-environment`) — это осознанная защита от поломки системных пакетов ОС. Если понадобится поставить Python-пакет прямо на хост (не в Docker) — используй venv:
```bash
python3 -m venv ~/.venv
source ~/.venv/bin/activate
pip install <пакет>
```
Либо `sudo apt install python3-<пакет>`, либо `pipx install <утилита>` для отдельных CLI-инструментов. Внутри Docker-контейнеров (все backend'ы в этом проекте) это не касается — там `pip install` работает как раньше.
